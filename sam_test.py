...
def readChar(image_path):
    ...
    boxes, texts, scores = [], [], []
    for line in result:
        #print("---",line[0][0],"-*"*5,type(line))
        for res in line:
            print("-##-"*5,res[0],"----",res[1][0],"----",res[1][1])
...
h_,w_  = 1685,1685
import os
read_path = '/home/shamsher/Desktop/ocr_trial/image_folder'
write_path = '/home/shamsher/Desktop/ocr_trial/save'
...
for cam in cam_id:
    ...
    for im in os.listdir(read_path):
        ...
        cv2.namedWindow("IMAGE_SEG",cv2.WINDOW_NORMAL)
        cv2.imshow("IMAGE_SEG",image)
        cv2.waitKey(0)
